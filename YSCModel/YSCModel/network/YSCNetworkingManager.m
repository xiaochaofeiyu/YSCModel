...
#import "YSCNetworkingManager.h"
#import "AFNetworking.h"
#import "AFNetworkActivityIndicatorManager.h"
#import "MBProgressHUD.h"

NSString *const kYSCNetworkStatusDidChanged = @"YSCNetworkStatusDidChanged";

static NSMutableArray *tasks;
@implementation YSCNetworkingManager

+ (YSCNetworkingManager *)sharedManager
{
    static YSCNetworkingManager *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[YSCNetworkingManager alloc] init];
    });
    return handler;
}
...
- (YSCURLSessionTask *)baseRequestType:(NSUInteger)type
                                     url:(NSString *)url
                                  params:(NSDictionary *)params
                                 success:(YSCResponseSuccess)success
                                    fail:(YSCResponseFail)fail
                                 showHUD:(BOOL)showHUD
{
    ...
    YSCURLSessionTask *sessionTask=nil;
...
- (YSCURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    YSCURLSessionTask *sessionTask = [manager POST:urlStr parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩图片
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                DLog(@"未知网络");
                [YSCNetworkingManager sharedManager].networkStats=StatusUnknown;
                
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                DLog(@"没有网络");
                [YSCNetworkingManager sharedManager].networkStats=StatusNotReachable;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                DLog(@"手机自带网络");
                [YSCNetworkingManager sharedManager].networkStats=StatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi: // WIFI
                
                [YSCNetworkingManager sharedManager].networkStats=StatusReachableViaWiFi;
                DLog(@"WIFI--%d",[YSCNetworkingManager sharedManager].networkStats);
                break;
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:kYSCNetworkStatusDidChanged object:nil];
    }];
    [mgr startMonitoring];
}
...
{
    BOOL reachable = NO;
    YSCNetworkStatus status = [YSCNetworkingManager sharedManager].networkStats;
    if (StatusReachableViaWiFi == status || StatusReachableViaWWAN == status) {
        reachable = YES;
    }
